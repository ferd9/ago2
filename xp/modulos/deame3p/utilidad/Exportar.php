	...
	*/
    ...
    {   $tipo   = is_numeric($valor);
        ...
        {   case 0: // El campo no es numerico
                    ...
                    $aFecha     = array();
                    if(preg_match("/(^[012]?[0-9]?|^[3]?[0-1]).([0-1]?[0-9]).([0-9]{4})$/",$valor,$aFecha))
                    {   
                        if(checkdate($aFecha[2],$aFecha[1],$aFecha[3]))
                        {   
                            $campo  = date ("Y-m-d",(mktime(0, 0, 0, $aFecha[2], $aFecha[1], $aFecha[3])));
                        }
                        else
                    ...
                    {   
                        if(preg_match("/(^[0-9]{4}).([0-1]?[0-9]).([0123][0-9]?)$/",$valor,$aFecha))
            ...
            case 1: // El campo es numerico
                    ...
                    {   // 5/08/2007 = 39299 Dia Testigo.
                        $campo      = (int)($valor) - 39299;
                        $campo  = date ("Y-m-d",(mktime(12, 0, 0, 8, 5, 2007)+($campo*60*60*24)));
                    }
                    break;
        }
    return $campo;  
	...
	public function char($campo,$valor)
	{
	   return $this->varchar($campo,$valor);
	}
	...
	{	$propiedades	= $this->datosPOST[$campo];
        ...
        if( $valor && strpos($conjuntoSet,$valor)!==false)
		{	$campo	= $valor;	}
		else
