            ...
            if ($period!=0) {
	            ...
	            ($_SESSION['wss']>=0 && $PMBP_SYS_VAR['last_scheduled_'.$_SESSION['wss']]>(time()-$period)) ) {
	                if($PMBP_SYS_VAR['scheduled_debug'])
	                    echo "No es tiempo para nuevas copias de seguridad todavía! Plazo fijado a 'En cada llamada' para la depuración.<br>\n";
	                return FALSE;
	            }
            }
        } else {
            if($PMBP_SYS_VAR['scheduled_debug'])
                echo "La clave de seguridad que está mal. Se debe tener el mismo valor que en global_conf.php. Crear un nuevo guión!<br>\n";
            return FALSE;
        }
    } else {
        if($PMBP_SYS_VAR['scheduled_debug'])
            echo "Ninguna base de datos o directorio se realiza copia de seguridad.<br>\n";
        return FALSE;
    }
    if($PMBP_SYS_VAR['scheduled_debug'])
        echo "Inicio copias de seguridad de las bases de datos ahora:<br>\n";
} else {
    $mode="web";
    include("login.php");
}
