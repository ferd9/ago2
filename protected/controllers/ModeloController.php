...
class ModeloController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','marca','limpiar'),
				'users'=>array('*'),
			),
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Modelo']))
		{
			$model->attributes=$_POST['Modelo'];
                        ...
                        $model->idmarca=$_POST['Modelo']['idmarca'];
                        $model->idcategoria=$_POST['Modelo']['idcategoria'];
	...
	protected function performAjaxValidation($model)
	{
        ...
        {
                $mar = new CDbCriteria;
                $mar->order = 'descripcion asc';
                $mar->condition = 'idcategoria = '.$_POST['Modelo']['idcategoria'];
                $datosMar = new Marca;


                $data = $datosMar->findAll($mar);

                echo CHtml::tag('option',array('value' => '0,0'),CHtml::encode('Seleccionar'),true);
                $data = CHtml::listData($data,'idmarca','descripcion');
                foreach($data as $id => $value)
                {
                    echo CHtml::tag('option',array('value' => $id.','.$_POST['Modelo']['idcategoria']),CHtml::encode($value),true);
